...
void put_glyph(char c, Image& fontMap, Face& font) {
    int i = c - 16;
    int w = texture_resolution / char_width,
        x = (i % w) * char_width,
        y = (i / w) * char_width;

    Image glyph = font.getCharBitmapBinary(c);
...
int main(int argc, char** argv) {
    ...
    Image fontMap = Image(texture_resolution, texture_resolution, Image::Type::Bitmap, 8);
    for(char c = 16 ; c != 127 ; ++c) {
        put_glyph(c, fontMap, f);
    }
    fontMap.save("font.png", Image::Format::Png);
}
